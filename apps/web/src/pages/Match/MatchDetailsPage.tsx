...
export default function MatchDetailsPage() {
    ...
    const { data: matchData, loading: matchLoading, error: matchError, refetch } = useMatchDetails(matchId);
    const { game, loading: gameLoading, error: gameError } = useGameDetails(matchData?.match?.game_id);
    ...
    const yourParticipant = participants.find((p) => p.user_id === state.user?.id);
    ...
    return (
        ...
        >
            ...
            <YStack gap="$2">
                {participants.map((p) => (
                    ...
                    >
                        ...
                        <Text>
                            {p.approved === null && <Hourglass />}
                            {p.approved === true && !isCreator && <BadgeCheck />}
                            {p.approved === false && <OctagonX />}
                        </Text>

                        {
                            p.approved === true && allAccepted ? (
                                <Select
                                ...
                                >
                                    <Select.Trigger
                                        ...
                                        jc="space-between" // space between label and icon
                                        px="$2"            // padding inside
                                        height={36}
                                    >
            ...
            {
                yourParticipant && yourParticipant.approved === null && !isCompleted && (
            ...
            {/* Creator Result Selection */}
            {
                isCreator && allAccepted && !isCompleted && (
                    <PrimaryButton
...
// http://localhost:5173/match/d722d07a-cf1a-492e-9aab-efe25a192e6a