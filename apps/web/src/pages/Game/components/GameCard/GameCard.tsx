...
import { GameImage, IconButton } from "../index";
import { useSaveGame, useRemoveGame } from "@checkmate/hooks";
...
export default function GameCard({
  ...
  return (
    <Card
      ...
      bg="$backgroundHover"
    >
      <GameImage src={imageUrl} rounded={true} />

      <Text fontWeight="700" numberOfLines={2} fontSize="$2" py={25}>
        {name}
      </Text>
      ...
      <PrimaryButton size="$2" onPress={() => navigate(`/game/${id}`)}>
        View Details
      </PrimaryButton>
    </Card>
  );
